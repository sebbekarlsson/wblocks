...
Website::Website(std::string directory) {
    ...
    this->site = nlohmann::json::parse(
            ResourceManager::get(this->dir + "/site.json")
            );

    std::cout <<
        "[Initializing]: Creating directory: " <<
        this->site["title"].get<std::string>() <<
        std::endl;
 ...
 */
std::string Website::replace_word(
        std::string text,
        std::string word,
